...
function setUserUI(user) {
  ...
  const topAvatarEl = $("topAvatar"); // might not exist now

  if (userNameEl) userNameEl.textContent = name;
  if (userEmailEl) userEmailEl.textContent = email;
  if (avatarEl) avatarEl.textContent = letter;
  if (topAvatarEl) topAvatarEl.textContent = letter; // ✅ safe
}
...
function buildMonthlyCalendar(dailyPnLMap, year, monthIndex) {
  ...
  // ✅ Mobile: 7 columns only (no weekly column)
  if (isMobile) {
  ...
  for (let week = 0; week < totalWeeks; week++) {
    ...
    // ✅ Desktop: keep your original weekly column cell
    if (!isMobile) {
    ...
    // ✅ Mobile: add a full-width weekly row UNDER the week (looks premium)
    if (isMobile) {
